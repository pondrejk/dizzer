...
## Velocity (& Extensionality & Scalability)

-- of production/publishing/visualization

Parallel to family photography. Importance of comparision.

Video on visiulal variables in animation and more research in this topic -- mac eachran

-- thickenss of durative code
-- other assessment -- events per unit of map time
-- time scale, time resolution

-- analysis in-memory without storing in a database

**Extensionality**

- spatial context as a separate form of extensionality
- extensionality by geocoding
- a different kind of change

**Scalability**

-- in meaning of scaleable systems?
-- how many levels?
-- how related to resolution
...
UNCERTAINTY mapping, where to put?

## Volume (& Exhaustivity & Scalability)

-- number of records, place on disc, storage requirements per record
-- cart cares only about the first
-- comperssion by cartography (recoverable)
-- cognitive stuff -- maceachran

**Exhaustivity**

-- n=all
-- not cognitively possible in visualization?
-- (big data bar chart, pie chart... joke)

-- weeding -- ML application area?

## Resolution

-- scale dependent viz -- fundamental stuff!!!
-- how to visual aggregation by scale
-- MAUP -- big data persistent?

-- what kinds? time/spatial/attribute

## Indexicality

-- not much to say, simply a prerequisite
...
-- weakest property, not really required

-- divide between structured and unstructured data
-- structured: fits to the relational database design
...
Variety in geospatial data -- research
-- metadata -- how influenced by Big Data?
-- structured data -- giving the spatial context
-- unstructured data -- thematic overlay
...
Some authors define a specific property of Big Data -- *veracity*, referring the messiness and trustworthiness of data, that is usually less controllable. To me, this is another aspect of variety, data vary not only in type and structure, but also in quality. This is can be the case for small data as well, however as @marr2014big hopes, "the volumes often make up for the lack of quality or accuracy."
...
## Interconnections of parameters

## Bonus: spatialities of big data

-- doesn't have to exist, but often do (some statistic), and if they do, what interesting you can find about it?
-- what geostatistics can say about BD? -- maybe it requires some compression algorithms for calculating parameters

## BD assessing tool
-- polar diagrams for classification?
-- a sw tool?
-- possible extensions to metadata profiles?
-- apply on data assessed by kitchin2016? -- get more precise parameters, rather than simple yes/no? -- what are stop values?
-- add parameters specific to spatial processing of such data -- as a prelude to cartographic processing: --geocodable?, --what kind of anchor geometry?, --metadata?
